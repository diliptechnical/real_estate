    ...
    <div class="card">
        ...
        <div class="card-body">
            <form action="{{ route("admin.property.store") }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="form-group col-sm-6 {{ $errors->has('floor_plan') ? 'has-error' : '' }}">
                <label for="price">Floor Plan (File size supported under 25MB ) </label>
                    <input type="file" id="plan_images" multiple name="plan_images" class="form-control" value="{{ old('plan_images', isset($category) ? $category->floor_plan : '') }}">
                    </div>
