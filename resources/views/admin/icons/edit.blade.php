...
<div class="table-container">
    ...
    <div class="card">
        ...
        <div class="card-body">
            <form action="{{ route("admin.icons.update", [$category->id]) }}" id="addform" method="POSt" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                <input type="hidden" id="id" name="id" class="form-control" value="{{  $category->id }}">
                <input type="hidden" id="url_root" name="url_root" class="form-control" value="{{ route("admin.icons.update", [$category->id]) }}">
                <div class="row">
                    ...
                    <input type="file" id="value" name="value" class="form-control" value="{{ old('image', isset($category) ? $category->image : '') }}">
                    <img src="{{config('app.url').'uploads/icons/'.$category->image}}" width="100">

                    <!-- <select id="value" name="value" class="form-control">
                        @foreach($icon as $ic)
                        <option value="{{$ic->id}}" style="background-image:url({{config('app.url').'uploads/icons/'.$ic->image }});">
                            {{$ic->image}}
                    </option>
                        @endforeach
                    </select> -->
